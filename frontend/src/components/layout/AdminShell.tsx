...
const NAV_SECTIONS = [
  {
    label: 'Workspace',
    items: [
      { to: '/admin', label: 'Dashboard', end: true, dot: 'blue' },
      { to: '/admin/appointments', label: 'Appointments', dot: 'amber' },
      { to: '/admin/jobs', label: 'Jobs', dot: 'green' },
      { to: '/admin/clients', label: 'Clients', dot: 'purple' },
    ],
  },
  {
    label: 'Finance',
    items: [
      { to: '/admin/accounting', label: 'Accounting', dot: 'blue' },
      { to: '/admin/inbox', label: 'Inbox', dot: 'coral' },
    ],
  },
  {
    label: 'Studio',
    items: [
      { to: '/admin/portfolio', label: 'Portfolio', dot: '' },
      { to: '/admin/settings', label: 'Settings', dot: '' },
    ],
  },
]
...
const DOT_BG: Record<string, string> = {
  ...
  coral:  'bg-red-100 dark:bg-red-900/40',
}

function NavDot({ color }: { color: string }) {
  return (
    <span className={cn('w-4 h-4 rounded-[4px] flex-shrink-0', color ? DOT_BG[color] : 'bg-border')} />
  )
}
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative text-muted-foreground hover:text-foreground hover:bg-muted/50 h-8 w-8"
      ...
      <nav className="flex flex-col gap-0.5 flex-1">
        {NAV_SECTIONS.map((section) => (
          <React.Fragment key={section.label}>
            ...
            {section.items.map(({ to, label, end, dot }) => (
              <NavLink
                key={to}
                to={to}
                end={end}
                ...
                className={({ isActive }) =>
                  cn(
                    'flex items-center gap-2 px-2 py-[7px] rounded-lg text-[13.5px] transition-all duration-100',
                    isActive
      ...
      <div className="mt-4 pt-4 border-t border-border flex flex-col gap-1">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 px-2 py-2 rounded-lg w-full text-left hover:bg-muted/50 transition-colors">
          ...
          <DropdownMenuContent side="top" align="start" className="w-48 bg-popover border text-popover-foreground z-[100]">
...
export function AdminShell() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex">
      ...
      <aside className="hidden md:flex flex-col w-[220px] fixed inset-y-0 left-0 bg-background border-r border-border px-3 py-6 z-30">
        <SidebarContent />
      </aside>
      ...
      {/* Main content */}
      <div className="flex-1 md:pl-[220px] min-h-screen flex flex-col">
        {/* Mobile top bar */}
